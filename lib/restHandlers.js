	...
	RestHandlers.prototype.genericResponse = function(method, object, responseBody, callback, rawRequest, rawResponse) {
		...
		if (_.isString(responseBody)) {
			try {
				...
				return callback(err, null, null, rawRequest, rawResponse);
			}
		}
		...
		if (responseBody.success === true) {
			if (_.isFunction(callback)) {
				return callback(null, responseBody.data, responseBody.additional_data, rawRequest, rawResponse);
			}
		}
			...
			if (_.isFunction(callback)) {
				return callback(errorObject, null, null, rawRequest, rawResponse);
			}
		}
